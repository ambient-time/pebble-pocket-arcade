...
def run(platform):
    ...
    try:
        ...
        button('Up');expect(d1=71)
        button('Select');expect(d2=0)
        ...
        frozen=current()['ticks'];time.sleep(1)
        assert current()['ticks']==frozen
        button('Back');expect(screen=0)
        deadline=time.monotonic()+20
        while current()['d3'] and time.monotonic()<deadline:time.sleep(.1)
        expect(d3=0);grab('after-exchange')
        assert current()['ticks']>0
        button('Select',hold=.6);expect(d3=1)
        time.sleep(.2)
        watch.send_packet(AppRunState(data=AppRunStateStop(uuid=APP)))
        time.sleep(.3)
        watch.send_packet(AppRunState(data=AppRunStateStart(uuid=APP)))
